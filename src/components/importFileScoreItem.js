...
export default Vue.extend({
  template: `<div>
            <div class="importLink" v-show="moduleName && tempUrl">
                <a :href="tempUrl" download="积分项列表-导入模板.xlsx" style="color:#108EE9;font-size:12px">1、下载列表-导入模板</a>
                <a style="color:#108EE9;font-size:12px;margin-left:20px" @click="uploadFile">2、上传导入文件</a>
                <input type="file" id="scoreitemsheetjs-input" accept=".xls,.xlsx" @change="onchange"  style="display:none"/>
            </div>
  ...
  beforeMount() {
    ...
    //   { headerName: "顶级编码", field: "topCode" },
    //   { headerName: "顶级名称", field: "topName" },
    //   { headerName: "排序", field: "sort" },
    //   { headerName: "分值说明", field: "desc" },
  ...
  watch: {
    closeImportDialog(val) {
      if (!val) {
        var elementId = document.getElementById("scoreitemsheetjs-input");
        elementId.value = "";
  ...
  methods: {
    ...
    // 上传按钮触发
    uploadFile() {
      document.getElementById("scoreitemsheetjs-input").click();
    },
    // 文件本地获取触发
    onchange: function(evt) {
      ...
      reader.onload = e => {
        ...
        if (this.moduleName == "scoreItemManager") {
          var params = {};
          var list = [];
          for (var b in arr) {
